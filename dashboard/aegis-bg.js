(function () {
  ...
  var css = [
    ...
    "#aegisBg .agb-img{position:absolute;inset:-8%;background:url('" + IMG + "') center 20%/cover no-repeat;opacity:.55;filter:saturate(1.2) contrast(1.05) drop-shadow(0 0 40px rgba(34,211,238,.4));animation:agbZoom 38s ease-in-out infinite;will-change:transform;transition:filter .8s}",
    "#aegisBg .agb-veil{position:absolute;inset:0;background:radial-gradient(70vw 60vh at 50% 30%,rgba(34,211,238,.10),transparent 60%),linear-gradient(180deg,rgba(7,10,18,.2) 0%,rgba(7,10,18,.46) 55%,rgba(7,10,18,.92) 100%)}",
    ...
    "#aegisBg .agb-glow{position:absolute;left:50%;top:26%;width:64vmin;height:64vmin;transform:translate(-50%,-50%);border-radius:50%;background:radial-gradient(circle,rgba(34,211,238,.24),transparent 66%);animation:agbPulse 5s ease-in-out infinite;transition:background .8s}",
  ...
  function apply(st) {
    ...
    if (alert) {
      ...
      glow.style.background = "radial-gradient(circle,rgba(34,211,238,.24),transparent 66%)";
